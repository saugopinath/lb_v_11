...
@section('content')
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Government of West Bengal Lakshmir Bhandar Scheme</h1>
          </div>
        </div>
      </div>
    </section>
      ...
      <div class="container-fluid">
        <div class="row">
          <div class="col-md-12">
            ...
            @if (!empty($application_id))
              <div class="card card-info">
                <div class="card-header">
                  <h3 class="card-title"><b>Application Information</b></h3>
                </div>
                <div class="card-body">
                  <div class="alert alert-info">
                    <strong>Application ID: {{$application_id}}</strong><br>
                    <strong>Beneficiary Name: {{$ben_name}}</strong>
                  </div>

                  <div id="forEditInfo">
                    @if($row->life_certificate_checked == 1)
                      @if($row->life_certificate_pass == 1)
                        <strong><i class="fas fa-check text-success"></i><span class="text-success">
                            Checked Bio-auth from Kadhya Sathi - Passed
                            @if(!empty($row->last_biometric))({{date('d/m/Y', strtotime($row->last_biometric))}})@endif</span></strong>
                        <br>
                      @else
                        <strong><i class="fas fa-times text-danger"></i><span class="text-danger"> Checked
                            Bio-auth from Kadhya Sathi:Not Passed @if(!empty($row->life_certificate_lastdatetime)) as on
                            {{date('d/m/Y', strtotime($row->life_certificate_lastdatetime))}} @endif</span></strong> <br>
                      @endif
                    @endif
                    ...
                    @if(($row->caste == 'SC' || $row->caste == 'ST') && $row->caste_certificate_checked == 1)
                      @if($row->caste_matched_with_certificate_no == 1)
                        <strong><i class="fas fa-check text-success"></i><span class="text-success">
                            Checked Caste Certificate Status:Passed @if(!empty($row->caste_certificate_check_lastdatetime)) as
                            on {{date('d/m/Y', strtotime($row->caste_certificate_check_lastdatetime))}}@endif</span></strong>
                        <br>
                      @else
                        <strong><i class="fas fa-times text-danger"></i><span class="text-danger"> Checked
                            Caste Certificate Status:Not Passed @if(!empty($row->caste_certificate_check_lastdatetime)) as on
                            {{date('d/m/Y', strtotime($row->caste_certificate_check_lastdatetime))}}@endif</span></strong>
                        <br>
                      @endif
                    @endif
                    ...
                    @if($aadhaar_no_checked == 1)
                      @if($row->aadhaar_no_checked_pass == 1)
                        <span class="text-success" style="font-size: 14px; font-weight: bold;"> <i class="fas fa-check"></i>
                          Checked Aadhaar Card Demographic Status: Passed as on
                          @if(!empty($row->aadhaar_no_checked_lastdatetime))
                          {{date('d-m-Y', strtotime($row->aadhaar_no_checked_lastdatetime))}} @endif
                          @if(($row->dob_is_match_kh != 1))
                            @if(!empty($row->dob_kh))
                              <span class="text-warning" style="font-size: 14px; font-weight: bold;">
                                But Applicant DOB @if(!empty($row->dob)) ({{date('d-m-Y', strtotime($row->dob))}})@endif not match
                                with DOB ({{date('d-m-Y', strtotime($row->dob_kh))}}) which is received from Khadya Sathi Api.
                              </span>
                            @endif
                          @endif
                        </span>
                      @elseif($row->aadhaar_no_checked_pass == 2)
                        <span class="text-warning" style="font-size: 14px; font-weight: bold;"> <i class="fas fa-times"></i>
                          Checked Aadhaar Card Demographic Status:@if(!empty($row->aadhaar_no_validation_msg))
                          {{$row->aadhaar_no_validation_msg}} @endif as on @if(!empty($row->aadhaar_no_checked_lastdatetime))
                          {{date('d-m-Y', strtotime($row->aadhaar_no_checked_lastdatetime))}}@endif</span>
                      @else
                        <span class="text-danger" style="font-size: 14px; font-weight: bold;"> <i class="fas fa-times"></i>
                          Checked Aadhaar Card Demographic Status: Not Passed as on
                          @if(!empty($row->aadhaar_no_checked_lastdatetime))
                          {{date('d-m-Y', strtotime($row->aadhaar_no_checked_lastdatetime))}}@endif</span>
                      @endif
                    @endif
            ...
            <div class="alert alert-danger print-error-msg" style="display:none;" id="errorDiv">
            ...
            <div id="life_certificate_pass" style="display:none;">
              <strong><i class="fas fa-check text-success"></i><span class="text-success">Checked Bio-auth from
                  Kadhya Sathi - Passed (<span id="life_certificate_pass_text"></span>)</span></strong>
            </div>

            <div id="life_certificate_n_pass" style="display:none;">
              <strong><i class="fas fa-times text-danger"></i><span class="text-danger">Checked Bio-auth from Kadhya
                  Sathi- Not Passed as on <span id="life_certificate_n_pass_text"></span></span></strong>
            </div>

            <div id="cast_certificate_pass" style="display:none;">
              <strong><i class="fas fa-check text-success"></i><span class="text-success">Checked Caste Certificate
                  Status:Passed as on <span id="cast_certificate_pass_text"></span></span></strong>
            </div>

            <div id="cast_certificate_n_pass" style="display:none;">
              <strong><i class="fas fa-times text-danger"></i><span class="text-danger">Checked Caste Certificate
                  Status:Not Passed as on <span id="cast_certificate_n_pass_text"></span></span></strong>
            </div>

            <div id="aadhaar_no_checked_pass" style="display:none;">
              <strong><i class="fas fa-check text-success"></i><span class="text-success">Checked Aadhaar Card
                  Demographic Status:Passed as on <span id="aadhaar_no_checked_pass_text"></span>
                  <span class="text-warning">
                    <span id="aadhar_message"> But Applicant DOB not match with DOB (<span
                        id="aadhaar_no_dob_pass_text"></span>) which is received from Khadya Sathi API.
                    </span>
                  </span></span></strong>
            </div>
            <div id="aadhaar_no_checked_n_pass" style="display:none;">
              <strong><i class="fas fa-times text-danger"></i><span class="text-danger">Checked Aadhaar Card
                  Demographic Status:Not Passed as on <span id="aadhaar_no_checked_n_pass_text"></span></span></strong>
            </div>
            ...
            <div class="card card-primary">
              ...
              <div class="card-body">
                ...
                <ul class="nav nav-tabs" id="myTab" role="tablist">
                  ...
                  <li class="nav-item" role="presentation">
                    <button class="nav-link" id="enclosure-tab" data-bs-toggle="tab" data-bs-target="#enclosure"
                      type="button" role="tab" aria-controls="enclosure" aria-selected="false">
                      <b>Enclosure List (Self Attested)</b>
                    </button>
                  </li>
                  <li class="nav-item" role="presentation">
                    <button class="nav-link" id="declaration-tab" data-bs-toggle="tab" data-bs-target="#declaration"
                      type="button" role="tab" aria-controls="declaration" aria-selected="false">
                ...
                <div class="tab-content" id="myTabContent">
                  ...
                  <div class="tab-pane fade show active" id="personal" role="tabpanel" aria-labelledby="personal-tab">
                    <form name="personal" id="personal" method="post" action="{{url('ajax_personal_entry_faulty')}}">
                      ...
                      <div class="row mb-3">
                        <div class="col-md-3">
                          ...
                          <input type="text" name="aadhar_no" id="aadhar_no" class="form-control NumOnly"
                            placeholder="Aadhar No." maxlength="12" value="{{isset($row->aadhar_no) ? $row->aadhar_no : ''}}"
                            tabindex="15" />
                          <span id="error_aadhar_no" class="text-danger"></span>
                        </div>
                        <div class="col-md-2 mt-4">
                          <button type="button" id="aadhar_dup_check"
                            class="btn btn-danger {{isset($row->aadhar_no) ? '' : 'disabled'}}">
                            Check Avialibility
                          </button>
                        </div>
                      </div>
                      ...
                      <div id="personal_div">
                        <div class="row mb-3">
                          ...
                          <div class="col-md-4">
                            <input type="text" name="entry_type_text" id="entry_type_text" class="form-control txtOnly"
                              placeholder="Form through Duare Sarkar camp" maxlength="200" readonly
                              value="Form through Duare Sarkar camp" tabindex="5" />
                        ...
                        <div class="row mb-3 duareSarkar" style="display:none;">
                          <div class="col-md-4">
                            <label class="form-label required-field">Duare Sarkar Registration no.</label>
                            <input type="text" name="duare_sarkar_registration_no" readonly
                              id="duare_sarkar_registration_no" class="form-control NumOnly"
                              placeholder="Duare Sarkar Registration no." maxlength="25"
                              value="{{isset($row->duare_sarkar_registration_no) ? $row->duare_sarkar_registration_no : ''}}"
                              tabindex="5" />
                            <span id="error_duare_sarkar_registration_no" class="text-danger"></span>
                          </div>
                          <div class="col-md-4">
                            <label class="form-label required-field">Duare Sarkar Date:</label>
                            <input type="date" name="duare_sarkar_date" readonly id="duare_sarkar_date"
                              class="form-control" placeholder="Duare Sarkar Date" max="<?php echo date("Y-m-d"); ?>"
                              value="{{isset($row->duare_sarkar_date) ? $row->duare_sarkar_date : ''}}" tabindex="10" />
                        ...
                        <div class="row mb-3">
                          <div class="col-md-4">
                            ...
                            <input type="text" name="first_name" id="first_name" class="form-control txtOnly"
                              placeholder="Name" maxlength="200" readonly value="{{($ben_name) ?? ''}}" tabindex="5" />
                        ...
                        <div class="row mb-3">
                          <div class="col-md-4">
                            ...
                            <input type="text" id="mobile_no" name="mobile_no" class="form-control NumOnly"
                              placeholder="Mobile No" maxlength="10" value="{{isset($row->mobile_no) ? $row->mobile_no : ''}}"
                              tabindex="185">
                        ...
                        <div class="row mb-3">
                          ...
                          <div class="col-md-4">
                            ...
                            <input type="date" name="dob" readonly id="dob" class="form-control" tabindex="25"
                              value="{{isset($row->dob) ? $row->dob : ''}}" max="{{$max_dob}}" min="{{$min_dob}}" />
                            <span id="error_dob" class="text-danger"></span>
                          </div>

                          <div class="col-md-4">
                            <label class="form-label">Age<span style=""> (as on {{$dob_base_date}})</span></label>
                            <input type="hidden" name="hidden_age" id="hidden_age"
                              value="{{isset($row->ben_age) ? $row->ben_age : ''}}">
                            <input type="text" readonly name="txt_age" id="txt_age" class="form-control NumOnly"
                              placeholder="Age" value="{{isset($row->ben_age) ? $row->ben_age : ''}}" maxlength="3"
                              tabindex="30" />
                        ...
                        <div class="row mb-3">
                          ...
                          <div class="col-md-4">
                            ...
                            <input type="text" name="father_last_name" id="father_last_name" class="form-control txtOnly"
                              ...
                              value="{{isset($row->father_lname) ? $row->father_lname : ''}}" tabindex="45" />
                        ...
                        <div class="row mb-3">
                          ...
                          <div class="col-md-4">
                            ...
                            <input type="text" name="mother_last_name" id="mother_last_name" class="form-control txtOnly"
                              placeholder="Last Name" maxlength="200"
                              value="{{isset($row->mother_lname) ? $row->mother_lname : ''}}" tabindex="60" />
                          ...
                          <div class="col-md-4">
                            ...
                            <select class="form-control" name="caste_category" id="caste_category" tabindex="70">
                              ...
                              @foreach(Config::get('constants.caste_lb') as $key => $val)
                                <option value="{{$key}}" @if(isset($row->caste)) @if(trim($row->caste) == $key) selected
                                @endif @endif>{{$val}}</option>
                              @endforeach
                            </select>
                            <span id="error_caste_category" class="text-danger"></span>
                          </div>

                          <div class="col-md-4" id="caste_certificate_no_section">
                            <label class="form-label required-field">SC/ST Certificate No.</label>
                            <input type="text" name="caste_certificate_no" id="caste_certificate_no" class="form-control"
                              placeholder="SC/ST Certificate No." maxlength="200"
                              value="{{isset($row->caste_certificate_no) ? $row->caste_certificate_no : ''}}"
                              tabindex="117" />
                        ...
                        <div class="row mb-3">
                          <div class="col-md-12 text-center">
                            ...
                            <br><br>
                            <span style="font-size:20px;" class="badge bg-info" id="dup_aadhaar_label"
                              style="{{isset($row->aadhar_no) ? 'display:none' : ''}}">Save & Next will be Available after
                              availability of Aadhaar</span>
                  ...
                  <div class="tab-pane fade" id="contact" role="tabpanel" aria-labelledby="contact-tab">
                    <form name="contact" id="contact" method="post" action="{{url('ajax_contact_entry_faulty')}}">
                      <input type="hidden" name="contactCount" id="contactCount" value="{{ $contactCount }}">
                      <input type="hidden" name="_token" value="{{ csrf_token() }}">

                      <div class="row mb-3">
                        <div class="col-md-12 ajax_loader" style="display:none;">
                        ...
                        <div class="col-md-4">
                          ...
                          <select name="district" id="district" class="form-control" tabindex="135">
                            <option value="">--Select --</option>
                            @foreach ($district_list as $dist)
                              <option value="{{$dist->district_code}}" @if(isset($row->dist_code))
                              @if($row->dist_code == $dist->district_code) selected @endif @endif>
                                {{trim($dist->district_name)}}</option>
                            @endforeach
                          </select>
                      ...
                      <div class="row mb-3">
                        ...
                        <div class="col-md-4" id="divUrbanCode">
                          ...
                          <select name="urban_code" id="urban_code" class="form-control" tabindex="144">
                            ...
                            @foreach(Config::get('constants.rural_urban') as $key => $val)
                              <option value="{{$key}}" @if(isset($row->rural_urban_id)) @if($row->rural_urban_id == $key)
                              selected @endif @endif>{{$val}}</option>
                            @endforeach
                          </select>
                        ...
                        <div class="col-md-4" id="divBodyCode">
                          <label class="form-label required-field">Block/Municipality/Corp.</label>
                          <select name="block" id="block" class="form-control" tabindex="150">
                            <option value="">--Select --</option>
                            @if(count($block_ulb_list) > 0)
                              @foreach ($block_ulb_list as $block)
                                <option value="{{$block->block_ulb_code}}" @if(isset($row->block_ulb_code))
                                @if($row->block_ulb_code == $block->block_ulb_code) selected @endif @endif>
                                  {{trim($block->block_ulb_name)}}</option>
                              @endforeach
                            @endif
                          </select>
                      ...
                      <div class="row mb-3">
                        <div class="col-md-4" id="divBodyCode">
                          <label class="form-label required-field">GP/Ward No</label>
                          <select name="gp_ward" id="gp_ward" class="form-control" tabindex="155">
                            <option value="">--Select --</option>
                            @if(count($gp_ward_list) > 0)
                              @foreach ($gp_ward_list as $gp)
                                <option value="{{$gp->gp_ward_code}}" @if(isset($row->gp_ward_code))
                                @if($row->gp_ward_code == $gp->gp_ward_code) selected @endif @endif>
                                  {{trim($gp->gp_ward_name)}}</option>
                              @endforeach
                            @endif
                          </select>
                        ...
                        <div class="col-md-4">
                          <label class="form-label required-field">Village/Town/City</label>
                          <input type="text" id="village" name="village" class="form-control special-char"
                            placeholder="Village/Town/City" maxlength="300"
                            value="{{isset($row->village_town_city) ? $row->village_town_city : ''}}" tabindex="160">
                          <span id="error_village" class="text-danger"></span>
                        </div>

                        <div class="col-md-4">
                          <label class="form-label">House / Premise No.</label>
                          <input type="text" id="house_premise_no" name="house_premise_no"
                            class="form-control special-char" placeholder="House / Premise No." maxlength="300"
                            value="{{isset($row->house_premise_no) ? $row->house_premise_no : ''}}" tabindex="165">
                      ...
                      <div class="row mb-3">
                        ...
                        <div class="col-md-4">
                          ...
                          <input type="text" id="pin_code" name="pin_code" class="form-control NumOnly"
                            placeholder="Pin Code" maxlength="6" value="{{isset($row->pincode) ? $row->pincode : ''}}"
                            tabindex="175">
                      ...
                      <div class="row mb-3">
                        <div class="col-md-12">
                          ...
                          <div class="float-end">
                            ...
                            <img style="display:none;" src="{{ asset('images/ZKZg.gif')}}" id="btn_contact_details_loader"
                  ...
                  <div class="tab-pane fade" id="bank" role="tabpanel" aria-labelledby="bank-tab">
                    <form name="bank" id="bank" method="post" action="{{url('ajax_bank_entry_faulty')}}">
                      <input type="hidden" name="bankCount" id="bankCount" value="{{ $bankCount }}">
                      <input type="hidden" name="_token" value="{{ csrf_token() }}">

                      <div class="row mb-3">
                        <div class="col-md-6">
                          <input type='hidden' id="bank_ifsc_code"
                            value="{{isset($row->bank_ifsc) ? $row->bank_ifsc : ''}}" />
                          <div class="modal_field_name" style="margin-right:2%;">IFS Code.:</div>
                          <div class="modal_field_value" id="bank_ifsc_code_k">
                            {{isset($row->bank_ifsc) ? $row->bank_ifsc : ''}}</div>
                        </div>

                        <div class="col-md-6">
                          <input type='hidden' id="name_of_bank" value="{{isset($row->bank_name) ? $row->bank_name : ''}}" />
                          <div class="modal_field_name" style="margin-right:2%;">Bank Name:</div>
                          <div class="modal_field_value" id="name_of_bank_k">{{isset($row->bank_name) ? $row->bank_name : ''}}
                          </div>
                        </div>
                      </div>

                      <div class="row mb-3">
                        <input type='hidden' id="bank_branch" value="{{isset($row->branch_name) ? $row->branch_name : ''}}" />
                        <div class="col-md-6">
                          <div class="modal_field_name" style="margin-right:2%;">Bank Branch Name.:</div>
                          <div class="modal_field_value" id="bank_branch_k">
                            {{isset($row->branch_name) ? $row->branch_name : ''}}</div>
                        </div>

                        <div class="col-md-6">
                          <input type='hidden' id="bank_account_number"
                            value="{{isset($row->bank_code) ? $row->bank_code : ''}}" />
                          <div class="modal_field_name" style="margin-right:2%;">Bank Account Number:</div>
                          <div class="modal_field_value" id="bank_account_number">
                            {{isset($row->bank_code) ? $row->bank_code : ''}}</div>
                        </div>
                      </div>
                      ...
                      <div class="row mb-3">
                        <div class="col-md-12">
                          ...
                          <div class="float-end">
                            ...
                            <img style="display:none;" src="{{ asset('images/ZKZg.gif')}}" id="btn_bank_details_loader"
                              width="150px" height="150px">
                          </div>
                        </div>
                      </div>
                  ...
                  <!-- Enclosure List Tab -->
                  <div class="tab-pane fade" id="enclosure" role="tabpanel" aria-labelledby="enclosure-tab">
                    <div class="card">
                      <div class="card-header">
                        <h4 class="card-title"><b>Enclosure List (Self Attested)</b></h4>
                      </div>
                      <div class="card-body">
                        @if(isset($encloser_list))
                          @foreach ($encloser_list as $doc_all)
                            <div class="row mb-3">
                              <div class="col-md-12">
                                <label
                                  class="fileLable_{{$doc_all['id']}} {{$doc_all['required'] == 1 ? 'required-field' : ''}}">{{ $doc_all['doc_name'] }}</label>
                                <div class="imageSize">(Image type must be {{ $doc_all['doc_type'] }} and image size max
                                  {{ $doc_all['doc_size_kb'] }}KB)</div>
                                <button type="button" id="doc_{{ $doc_all['id'] }}_{{ $doc_all['is_profile_pic'] }}"
                                  name="encolerModal" class="btn btn-info encloserModal btnEnc">Upload</button>
                                <span id="download_{{ $doc_all['id']}}"
                                  style="{{$doc_all['can_download'] == 1 ? '' : 'display:none'}}">
                                  &nbsp;&nbsp;<button type="button"
                                    id="docDownload_{{ $doc_all['id'] }}_{{ $doc_all['is_profile_pic'] }}"
                          ...
                          <div class="col-md-12">
                            <div class="float-start">
                              <button type="button" name="previous_btn_experience_details"
                                id="previous_btn_experience_details"
                                class="btn btn-info btn-lg">Previous</button>&nbsp&nbsp&nbsp
                            </div>
                            <div class="float-end">
                  ...
                  <div class="tab-pane fade" id="declaration" role="tabpanel" aria-labelledby="declaration-tab">
                    <form name="declaration" id="declaration" method="post"
                      action="{{url('ajax_declaration_entry_faulty')}}">
                      <input type="hidden" name="otherCount" id="otherCount" value="{{ $otherCount }}">
                      <input type="hidden" name="_token" value="{{ csrf_token() }}">

                      <div class="row mb-3">
                        <div class="col-md-12" tabindex="360">
                          <div class="form-check mb-2">
                            <input class="form-check-input" type="checkbox" name="is_resident" id="is_resident" value="1"
                              @if(isset($row->is_resident)) @if($row->is_resident == 1) checked @endif @endif>
                          ...
                          <div class="form-check mb-2">
                            <input class="form-check-input" type="checkbox" id="earn_monthly_remuneration"
                              name="earn_monthly_remuneration" value="1" @if(isset($row->earn_monthly_remuneration))
                              @if($row->earn_monthly_remuneration == 1) checked @endif @endif>
                            <label class="form-check-label">
                          ...
                          <div class="form-check mb-2">
                            <input class="form-check-input" type="checkbox" id="info_genuine_decl"
                              name="info_genuine_decl" value="1" @if(isset($row->info_genuine_decl))
                              @if($row->info_genuine_decl == 1) checked @endif @endif>
                            <label class="form-check-label">
                              That all the information and documents submitted by me are correct/ genuine. In case any of
                              the
                              information/ document is found to be false, penal action shall be taken against me and the
                              benefit will be
                              terminated.
                            </label>
                          ...
                          <div class="form-check mb-2">
                            <input class="form-check-input" type="checkbox" id="av_status" name="av_status" value="TRUE"
                              checked>
                            <label class="form-check-label" id='aadhar-text'>
                              I give consent to the use of the Aadhaar No.for authenticating my identity for social
  ...
  <!-- Preview Modal -->
  <div class="modal fade" id="confirm-submit" tabindex="-1" aria-labelledby="confirmSubmitLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="confirmSubmitLabel" style="text-align: center;">Confirm Submit</h5>
          ...
          <div class="section1">
            ...
            <div class="row">
              <div class="form-group col-md-12">
                ...
                <br />
                <label>
                  <i class="fas fa-check" aria-hidden="true"></i> That I do not earn any monthly remuneration from any
                  regular Government job
                  <br />
                </label>
                <br />
                <label>
                  <i class="fas fa-check" aria-hidden="true"></i> That all the information and documents submitted by me
                  are correct/ genuine. In case any of the
                  information/ document is found to be false, penal action shall be taken against me and the benefit will
                  be
                  terminated. <br />
                </label>
                <br />
        ...
        <div class="modal-footer justify-content-center">
          <div><img src="{{ asset('images/ZKZg.gif')}}" id="submit_loader" width="150px" height="150px"
              style="display:none;"></div>
          <button type="button" class="btn btn-secondary btn-lg" data-bs-dismiss="modal">Cancel</button>
          <button type="button" id="submit" value="Submit" class="btn btn-success btn-lg">Submit</button>
  ...
  <!-- Enclosure Upload Modal -->
  <div class="modal fade" id="encolser_modal" tabindex="-1" aria-labelledby="encolserModalLabel" aria-hidden="true">
  ...
  <script>
    ...
    $(document).ready(function () {
      ...
      $('#btn_personal_details').click(function () {
        ...
        if (error_entry_type != '' || error_duare_sarkar_registration_no != '' || error_duare_sarkar_date != '' || error_first_name != '' || error_dob != '' || error_gender != '' || error_txt_age != ''
        ...
        //if( valid==0 )
        {
          ...
          var data = $.param({ "_token": '{{ csrf_token() }}', "application_id": $("#commonfield #application_id").val() }) + "&" + $("#personal").serialize();
          $.ajax({
            type: 'POST',
            url: '{{ url('ajax_personal_entry_faulty') }}',
            dataType: 'json',
            data: data,
            beforeSend: function () {
              $('#btn_personal_details_loader').show();
      ...
      $('#aadhar_dup_check').click(function () {
        ...
        //return false;
        var data = $.param({ "_token": '{{ csrf_token() }}', "application_id": $("#commonfield #application_id").val(), "aadhar_no": $("#aadhar_no").val() });
        $.ajax({
          type: 'POST',
          url: '{{ url('faulty_ajax_check_dup_aadhar') }}',
          dataType: 'json',
          data: data,
          beforeSend: function () {
      ...
      $('#btn_contact_details').click(function () {
        ...
        //if(valid==0)
        {
          ...
          var data = $.param({ "_token": '{{ csrf_token() }}', "application_id": $("#commonfield #application_id").val() }) + "&" + $("#contact").serialize();

          $.ajax({
            type: 'POST',
            url: '{{ url('ajax_contact_entry_faulty') }}',
            dataType: 'json',
            data: data,
            beforeSend: function () {
              $('#btn_contact_details_loader').show();
      ...
      $('#bank_ifsc_code').blur(function () {
        ...
        if ($ifscRGEX.test($ifsc_data)) {
          ...
          $.ajax({
            type: 'POST',
            url: '{{ url('ajax_bank_entry_faulty') }}',
            data: {
              ifsc: $ifsc_data,
              _token: '{{ csrf_token() }}',
            },
      ...
      $('#btn_bank_details').click(function () {
        ...
        var data = $.param({ "_token": '{{ csrf_token() }}', "application_id": $("#commonfield #application_id").val() }) + "&" + $("#bank").serialize();
        $.ajax({
          type: 'POST',
          url: '{{ url('ajax_bank_entry_faulty') }}',
          dataType: 'json',
      ...
      $('#btn_encloser_details').on('click', function () {
        ...
        var data = $.param({ "_token": '{{ csrf_token() }}', "application_id": $("#commonfield #application_id").val() });
        $.ajax({
          type: 'POST',
      ...
      $("#uploadForm").on('submit', function (e) {
        ...
        $.ajax({
          ...
          type: 'POST',
          dataType: 'json',
          url: '{{ url('ajax_encloser_faulty_entry') }}',
          data: formData,
          contentType: false,
          cache: false,
          processData: false,
