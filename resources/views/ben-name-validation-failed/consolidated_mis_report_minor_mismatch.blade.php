          ...
          <div> <!-- class="box box-primary" -->
            <div class="box-header with-border">
             <h3 class="box-title"><b>
              ...
              Consolidated Mismatch MIS Report
...
<div id="divScrool"> 
         ...
         <thead>
              ...
              <tr> 
                <th id=""  rowspan="2" style="text-align:center">Sl No.(A)</th>
                <th id="location_id" rowspan="2" style="text-align:center">District</th>
                <th colspan="5" style="text-align:center">Matching Score 90%-100%</th> 
                <th colspan="5" style="text-align:center; background-color: aliceblue;">Matching Score 40%-89%</th> 
                <th colspan="2" style="text-align:center">Matching Score 0%-39%</th> 
                {{-- <th  rowspan="2" style="text-align:center; background-color: aliceblue;">Rejected(O)</th> --}}
              </tr> 
              <tr>  
                <th>Processed with Minor Mismatch(C)</th> 
                <th>Rejected(D)</th>
                <th>Approval Pending at District(E)</th>
                <th>Action Pending at Block(F)</th>
                <th>Total(G=C+D+E+F)</th>
 
                <th>Processed with Minor Mismatch(H)</th> 
                <th>Rejected(I)</th>
                <th>Action Pending at District(J)</th>
                <th>Action Pending at Block(K)</th>
                <th>Total(L=H+I+J+K)</th>
 
                <th>Total Match(M)</th> 
                <th>Rejected(N)</th>
                {{-- <th>Action Pending at District(L)</th>
                <th>Action Pending at Block(M)</th>
                <th>Total(N=J+K+L+M)</th>         --}}
              </tr> 
          </thead>
        <tbody>
...
$(document).ready(function(){
  ...
  $('.sidebar-menu #mis-report-console-mismatch').addClass("active"); 
  $('#submit_loader1').show();
  loadDataTable();
  $(".exportToExcel").click(function(e){
    ...
    exclude: ".noExl",
    name: "Worksheet Name",
    filename: "Lakshmir Bhandar Consolidated Mismatch Mis Report", //do not include extension
  ...
  var caste_category=$('#caste_category').val();
     ...
     $('#search_details').hide();
        $.ajax({
                type: 'post',
                dataType:'json',
                url: '{{ route('getConsolidatedMismatchReport') }}',
                data: {
                //   ds_phase: ds_phase,
                  district: district,
                ...
                success: function (data) {
                  ...
                  // alert(data.report_geneartion_time);
                  if(data.return_status){
                    $('#search_details').show();
                    $("#heading_msg").html("<h4><b>"+data.heading_msg+"</b></h4>");
                    $("#heading_excel").html("<b>"+data.heading_msg+"</b>");
                    $("#fotter_excel").html("Report Generated On: <b>"+data.report_geneartion_time+"</b>");
                   ...
                   var fotter_11=0;var fotter_12=0;var fotter_13=0;var fotter_14=0;var fotter_15=0;
                   $.each(data.row_data, function(i, item) {
                     var total_mismatch_0_39 = isNaN(parseInt(item.total_mismatch_0_39)) ? 0 : parseInt(item.total_mismatch_0_39);
                     var rejected_0_39 = isNaN(parseInt(item.rejected_0_39)) ? 0 : parseInt(item.rejected_0_39);

                     var yet_to_action_40_89 = isNaN(parseInt(item.yet_to_action_40_89)) ? 0 : parseInt(item.yet_to_action_40_89);
                     var approval_pending_40_89 = isNaN(parseInt(item.approval_pending_40_89)) ? 0 : parseInt(item.approval_pending_40_89);
                     var minor_mismatch_40_89 = isNaN(parseInt(item.minor_mismatch_40_89)) ? 0 : parseInt(item.minor_mismatch_40_89);
                     var rejected_40_89 = isNaN(parseInt(item.rejected_40_89)) ? 0 : parseInt(item.rejected_40_89);
                     var total_40_89 = yet_to_action_40_89+approval_pending_40_89+minor_mismatch_40_89+rejected_40_89;

                     var yet_to_action_90_100 = isNaN(parseInt(item.yet_to_action_90_100)) ? 0 : parseInt(item.yet_to_action_90_100);
                     var approval_pending_90_100 = isNaN(parseInt(item.approval_pending_90_100)) ? 0 : parseInt(item.approval_pending_90_100);
                     var minor_mismatch_90_100 = isNaN(parseInt(item.minor_mismatch_90_100)) ? 0 : parseInt(item.minor_mismatch_90_100);
                     var rejected_90_100 = isNaN(parseInt(item.rejected_90_100)) ? 0 : parseInt(item.rejected_90_100);
                     var total_90_100 = yet_to_action_90_100+approval_pending_90_100+minor_mismatch_90_100+rejected_90_100;

                    //  var rejected = isNaN(parseInt(item.rejected)) ? 0 : parseInt(item.rejected);

                    //  Column Add in Footer
                    fotter_1 = fotter_1 + minor_mismatch_90_100;
                    fotter_2 = fotter_2 + rejected_90_100;
                    fotter_3 = fotter_3 + approval_pending_90_100;
                    fotter_4 = fotter_4 + yet_to_action_90_100;
                    fotter_5 = fotter_5 + total_90_100;

                    fotter_6 = fotter_6 + minor_mismatch_40_89;
                    fotter_7 = fotter_7 + rejected_40_89;
                    fotter_8 = fotter_8 + approval_pending_40_89;
                    fotter_9 = fotter_9 + yet_to_action_40_89;
                    fotter_10 = fotter_10 + total_40_89;

                    fotter_11 = fotter_11 + total_mismatch_0_39;
                    fotter_12 = fotter_12 + rejected_0_39;
                    
                    // fotter_13 = fotter_13 + rejected;

                     table.append("<tr><td>"+(i+1)+"</td><td>"+item.block_subdiv_name+"</td><td>"+minor_mismatch_90_100+"</td><td>"+rejected_90_100+"</td><td>"+approval_pending_90_100+"</td><td>"+yet_to_action_90_100+"</td><td>"+total_90_100+"</td><td>"+minor_mismatch_40_89+"</td><td>"+rejected_40_89+"</td><td>"+approval_pending_40_89+"</td><td>"+yet_to_action_40_89+"</td><td>"+total_40_89+"</td><td>"+total_mismatch_0_39+"</td><td>"+rejected_0_39+"</td></tr>");
                      //slno++;

                  });
                  
                  $("#example > tfoot #fotter_id").html("<th></th><th>Total</th><th>"+fotter_1+"</th><th>"+fotter_2+"</th><th>"+fotter_3+"</th><th>"+fotter_4+"</th><th>"+fotter_5+"</th><th>"+fotter_6+"</th><th>"+fotter_7+"</th><th>"+fotter_8+"</th><th>"+fotter_9+"</th><th>"+fotter_10+"</th><th>"+fotter_11+"</th><th>"+fotter_12+"</th>");
                  //$('#example tbody').empty();
                   $("#example").show();
