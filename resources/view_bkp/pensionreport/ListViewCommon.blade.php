 ...
 <section class="content">
   ...
   <div class="box">
     ...
     <div class="box-body">
       ...
       <div class="alert print-error-msg" style="display:none" id="CrudModal">
        <button type="button" class="close"  aria-label="Close" onclick="closeError('CrudModal')"><span aria-hidden="true">&times;</span></button>
        <ul></ul></div>
   ...
   <script type="text/javascript">
     ...
     if (application_type == 'F') {
       ...
       $('.sidebar-menu #CompletedApplication').addClass("active");
     }
    ...
    $('#example_filter input')
     ...
     var columns = [{ "data": "id","defaultContent":"" },
     ...
     table = $('#example').DataTable({
       ...
       "ajax": {
         ...
         type: "POST",
         data   : function( d ) {
          d.scheme_code= $('#scheme_code').val();
          d.application_type= $('#application_type').val();
          d.district_code_fk= $('#district_code_fk').val();
          d.rural_urban_fk= $('#rural_urban_fk').val();
          d.block_munc_corp_code_fk= $('#block_munc_corp_code_fk').val();
          d.gp_ward_code_fk= $('#gp_ward_code_fk').val();
          d.mapping_column= $('#mapping_column').val();
          d.mapping_column_val= $('#mapping_column_val').val();
          d.mapping_rural_urban= $('#mapping_rural_urban').val();
     ...
     $('#scheme_code').change(function() {
       if ($(this).val() != '') {
         $("#searchDiv").hide();
         $('#ajax_loader').html('<img  src="{{ asset('images/ZKZg.gif') }}" width="50px" height="50px"/>');
     ...
     $('.client-js-urban').change(function() {
       if ($(this).val() != '') {
         ...
         if (error_found == 0) {
           $('#block_munc_corp_code').val('');
           $('#gp_ward').val('');
           $('#error_block_munc_corp_code').html('<img  src="{{ asset('images/ZKZg.gif') }}" width="50px" height="50px"/>');
           $.ajax({
             type: 'POST',
             url: url,
             data: {
     ...
     $('.client-js-localbody').change(function() {
       if ($(this).val() != '') {
         ...
         if (error_found == 0) {
           $('#gp_ward_code').val('');
           $('#error_gp_ward_code').html('<img  src="{{ asset('images/ZKZg.gif') }}" width="50px" height="50px"/>');
           $.ajax({
             type: 'POST',
             url: url,
             data: {
     ...
     function ViewApplicantModal(scheme_code, applicant_id, role_code, code = NULL) {
       ...
       $('#modal_data').html('<img align="center" src="{{ asset('images/ZKZg.gif') }}" width="50px" height="50px"/>');
       $.ajaxSetup({
         headers: {
           'X-CSRF-TOKEN': "{{csrf_token()}}"
         }
       ...
       $.ajax({
         ...
         success: function(data) {
            if(data.return_status){
               ...
               printMsg(data.return_msg,'0','CrudModal');
     ...
     $('#revert_Button').click(function(e) {
       ...
       $.ajax({
         ...
         success: function(data) {
            if(data.return_status){
               $('#example').DataTable().ajax.reload();
               printMsg(data.return_msg,'1','CrudModal');
