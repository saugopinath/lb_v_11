...
class WorkflowFaultyController extends Controller
{
  ...
  public function applicationdetails(Request $request)
  {
    if ($this->shemeSessionCheck($request)) {
      ...
      $personal_table = $getModelFunc->getTableFaultyWOutDoc($district_code, '', 1, 1);
      ...
      $aadhar_table = $getModelFunc->getTableFaultyWOutDoc($district_code, '', 2, 1);
      ...
      $verification_allowded = intval($allowded_arr->faulty_verification);
      $approval_allowded = intval($allowded_arr->faulty_approval);
      if ($is_first) {   // First Level Verifier   	
        ...
        /****************************District************************* */ 
          ...
          return view('processApplicationFaulty/pension_list', ['nhm_employee_details' => $rows, 'reject_revert_reason' => $reject_revert_reason, 'sessiontimeoutmessage' => $errormsg['sessiontimeOut']]);
          ...
          if ($is_urban == 1) {
            ...
            if (request()->ajax()) {
              ...
              $query = $personal_modal->where($condition)->wherenull($personal_table . '.next_level_role_id')->whereNotNull($personal_table . '.enquiry_officer_name')->where($personal_table . '.enq_iseligible', 1);
              $query = $query->join($contact_table, $contact_table . '.application_id', '=', $personal_table . '.application_id');
              $query = $query->join($bank_table, $bank_table . '.application_id', '=', $personal_table . '.application_id');

              if (!empty($request->filter_1)) {
                $query = $query->where('block_ulb_code', $request->filter_1);
              }
              if (!empty($request->filter_2)) {
                $query = $query->where('gp_ward_code', $request->filter_2);
              }
              ...
              if (empty($serachvalue)) {
                ...
                if (preg_match('/^[0-9]*$/', $serachvalue)) {
                  ...
                  //$totalRecords = $query->count();
                  $totalRecords = $query->count($personal_table . '.application_id');
                  $data = $query->orderBy($personal_table . '.ben_fname')->orderBy($contact_table . '.gp_ward_name')->offset($offset)->limit($limit)->get(
                    [
                      'application_id',
                      'ben_fname',
                      'ben_mname',
              ...
              return datatables()->of($data)
                ...
                ->rawColumns(['view', 'id', 'name', 'bank_code'])
                ->make(true);
            } else {
              return view('processApplicationFaulty/linelisting_verified_subdiv')
        ...
        /****************************Block************************* */ 
          ...
          if (request()->ajax()) {
            ...
            $query = $personal_modal->where($condition)->wherenull($personal_table . '.next_level_role_id')->where($personal_table . '.is_final', 1)->whereNotNull($personal_table . '.enquiry_officer_name')->where($personal_table . '.enq_iseligible', 1);
            $query = $query->join($contact_table, $contact_table . '.application_id', '=', $personal_table . '.application_id');
            $query = $query->join($bank_table, $bank_table . '.application_id', '=', $personal_table . '.application_id');
            if (!empty($request->filter_1)) {

              $query = $query->where('gp_ward_code', $request->filter_1);
            }
            if (!empty($request->caste_category)) {
            ...
            if (empty($serachvalue)) {
              //$totalRecords = $query->count();
              // $totalRecords = $query->count($personal_table . '.application_id');
              $data = $query->orderBy($personal_table . '.ben_fname')->orderBy($contact_table . '.gp_ward_name');
              // $filterRecords = count($data);
            } else {
              if (preg_match('/^[0-9]*$/', $serachvalue)) {
                $query = $query->where(function ($query1) use ($serachvalue, $personal_table) {
            ...
            return view('processApplicationFaulty/linelisting_verified')
        ...
        /****************************District************************* */ else if ($mappingLevel == "District") {
          ...
          if (request()->ajax()) {
            ...
            if (!empty($request->search['value']))
              ...
              $serachvalue = '';
            $query = $personal_modal->where($condition)->where('next_level_role_id', $role_id)->whereNotNull($personal_table . '.enquiry_officer_name')->where($personal_table . '.enq_iseligible', 1)->where($personal_table . '.ver_iseligible', 1)->where($personal_table . '.is_aadhar_dup', FALSE);
            $query = $query->join($contact_table, $contact_table . '.application_id', '=', $personal_table . '.application_id');
            $query = $query->join($bank_table, $bank_table . '.application_id', '=', $personal_table . '.application_id');
            if (!empty($request->filter_1)) {
            ...
            return datatables()->of($data)
              ...
              ->rawColumns(['view', 'check', 'id', 'name', 'bank_code'])
              ->make(true);
          }


          return view('processApplicationFaulty/linelisting_approved')->with('duty_level', $duty_level)
          ...
          if ($is_urban == 1) {
            ...
            return view('processApplicationFaulty/linelisting_approved', ['datas' => $rows, 'dist_code' => $district_code, 'reject_revert_reason' => $reject_revert_reason, 'sessiontimeoutmessage' => $errormsg['sessiontimeOut'], 'dob_base_date' => $dob_base_date]);
          }
        }
      }
    } else {
  ...
  public function forwardDataFaulty(Request $request)
  {
    //dd($request->all());
    ...
    $ver_iseligible = $request->ver_iseligible;

    if ($designation_id == 'Operator' && $opreation_type == 'V') {
      $rules = [
        'ver_iseligible' => 'required|in:1'
      ];
      $attributes = array();
      $messages = array();
      $attributes['ver_iseligible.required'] = "Please check the checkbox that applicant is eligible for  Lakshmir Bhandar scheme  and for <br/>1. Swasthya sathi 2. Aadhar Card 3. Caste";
      $attributes['ver_iseligible.in'] = "Please check the checkbox That applicant is eligible for  Lakshmir Bhandar scheme  and for <br/>1. Swasthya sathi 2. Aadhar Card 3. Caste";

      $validator = Validator::make($request->all(), $rules, $messages, $attributes);
      if (!$validator->passes()) {
        $return_status = 0;
        $return_msg = $validator->errors()->all();
        return response()->json(['return_status' => 0, 'return_msg' => $return_msg]);
      }
    }
    ...
    $Table = $getModelFunc->getTableFaultyWOutDoc($district_code, $this->source_type, 2, 1);
    ...
    if ($opreation_type == 'A') { //echo 1;die;
      $approval_allowded = DistrictEntryMapping::where('faulty_approval', true)->where('district_code', $district_code)->count();
      ...
      $verification_allowded = DistrictEntryMapping::where('faulty_verification', true)->where('district_code', $district_code)->count();
      ...
      if ($is_bulk == 0) {
        ...
        $row_list = $personal_model->select('application_id')->whereIn('application_id', $applicant_id_in)->whereNull('next_level_role_id')->where('created_by_dist_code', $request->session()->get('distCode'))->get();
        if (count($row_list) != count($applicant_id_in)) {
          return response()->json(['return_status' => 0, 'return_msg' => 'Applicant Id Not Valid']);
        }
      }
    ...
    if ($designation_id == 'Verifier' && $opreation_type == 'V') {
      $input = [
    ...
    try {
      ...
      if ($is_bulk == 1) {
        ...
        foreach ($row_list as $app_row) {
          ...
          $accept_reject_model->op_type = 'F' . $opreation_type;
      ...
      if ($opreation_type == 'A' || $opreation_type == 'R') {
        ...
        if ($is_bulk == 1) { //echo 1;
          ...
          if ($opreation_type == 'A') { //echo 2;
            $fun_return = DB::select("select lb_scheme.beneficiary_approve_faulty_final(in_application_id => $in_pension_id,in_action_by => $user_id,in_ip_address => '" . request()->ip() . "', in_action_type => '" . class_basename(request()->route()->getAction()['controller']) . "')");

          } else { //echo 3;
            //$reject_fun = DB::select("select lb_scheme.beneficiary_rejected_faulty_final(" . $in_pension_id . ")");
            $reject_fun = DB::select("select lb_scheme.beneficiary_rejected_faulty_final(in_application_id => $in_pension_id,in_action_by => $user_id,in_ip_address => '" . request()->ip() . "', in_action_type => '" . class_basename(request()->route()->getAction()['controller']) . "')");
  ...
  public function getBenViewAadharDataFaulty(Request $request)
  { //echo 1;die;
    $statusCode = 200;
    $response = [];
    $benid = $request->benid;
    ...
    try {
      if (!empty($benid)) {
        ...
        $Table = $getModelFunc->getTableFaultyWOutDoc($district_code, $this->source_type, 2);
  ...
  public function getBenViewInvestigatorData(Request $request)
  { //echo 1;die;
    $statusCode = 200;
    $response = [];
    if (!$request->ajax()) {
  ...
  function ajaxGetEncloserFaulty(Request $request)
  {
    $scheme_id = $this->scheme_id;
