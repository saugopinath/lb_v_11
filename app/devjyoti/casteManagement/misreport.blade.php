            ...
            <div class="tab-content" style="margin-top:16px;">
             ...
             <div class="tab-pane active" id="personal_details">
              <div class="panel panel-default">
               <div class="panel-heading"><h4><b>Caste Info Modification Mis Report </b></h4></div>
...
<button class="btn btn-info exportToExcel" id=exportToExcel style="display:none;" type="button" >Export to Excel</button><br/><br/><br/>
<button class="btn btn-info exportToExcel_new" id=exportToExcel_new  style="display:none;" type="button" >Export to Excel</button><br/><br/><br/>
<div id="divScrool"> 
              <table id="example" class="table table-striped table-bordered table2excel" style="width:100%">
              <thead>
              <tr>
              <td colspan="22" align="center" style="display:none;" id="heading_excel">Heading</td>
              </tr> 
              <tr > 

              <th id="" >Sl No.(A)</th>
              <th id="location_id" >District</th>
              {{-- <th colspan="7">CAST Modification FOR ST </th>
              <th colspan="7">CAST Modification FOR SC (F)</th> --}}
              ...
              <tr id="fotter_id"></tr>
              <tr>
              <td colspan="22" align="center" style="display:none;" id="fotter_excel">Heading</td>
              </tr> 
              </tfoot>
              </table>


              <table id="example_new" class="table table-striped table-bordered table3excel" style="width:100%">
                <thead>
                <tr>
                <td colspan="22" align="center" style="display:none;" id="heading_excel_new">Heading</td>
                </tr> 
               
  
                  <tr>
                    <th id="">Sl No.(A)</th>
                    <th id="location_id_new">District</th>
      
                    <th colspan="7" style="text-align: center">OTHERS TO SC </th>
                    <th colspan="7" style="text-align: center">OTHERS TO ST </th>
                    </tr>
                    ...
                    <th>Total beneficiaries on which caste info modification has been made(J)</th>
                    <th>Total Verified(K)</th>
                    <th>Total Yet to Be Verified(L)</th>
                    <th>Total Approved(M)</th>
                    <th>Total Yet to Be Approved(N)</th>
                    <th>Total Rejected(O)</th>
                    <th>Total Reverted(P)</th>
                    </tr>
                ...
                <tr id="fotter_id"></tr>
                <tr>
                <td colspan="22" align="center" style="display:none;" id="fotter_excel_new">Heading</td>
                </tr> 
                </tfoot>
                </table>
                ...
                success: function (data) {
                  ...
                  if(data.visible==0)
                  {
                   ...
                   $.each(data.row_data, function(i, item) {
                     ...
                     var total_yet_tobe_verified = isNaN(parseInt(item.total_yet_tobe_verified)) ? 0 : parseInt(item.total_yet_tobe_verified);
                     var total_approved = isNaN(parseInt(item.total_approved)) ? 0 : parseInt(item.total_approved);
                     var total_yet_tobe_approved = isNaN(parseInt(item.total_yet_tobe_approved)) ? 0 : parseInt(item.total_yet_tobe_approved);
                     var total_rejected = isNaN(parseInt(item.total_rejected)) ? 0 : parseInt(item.total_rejected);
                     var total_reverted = isNaN(parseInt(item.total_reverted)) ? 0 : parseInt(item.total_reverted);
                     ...
                     table.append("<tr><td>"+(i+1)+"</td><td>"+item.location_name+"</td><td>"+tot_ben+"</td><td>"+total_verified1+"</td><td>"+total_yet_tobe_verified+"</td><td>"+total_approved+"</td><td>"+total_yet_tobe_approved+"</td><td>"+total_rejected+"</td><td>"+total_reverted+"</td></tr>");
                      //slno++;

                  });
                  
                  $("#example > tfoot #fotter_id").html("<th></th><th>Total</th><th>"+fotter_1+"</th><th>"+fotter_2+"</th><th>"+fotter_3+"</th><th>"+fotter_4+"</th><th>"+fotter_5+"</th><th>"+fotter_6+"</th><th>"+fotter_8+"</th>");
                  //$('#example tbody').empty();
                   $("#example").show();
                   $("#example_new").hide();
                   $("#exportToExcel").show();
                  ...
                  else if(data.visible==1)
                  {
                   ...
                   var fotter_7=0;
                    ...
                    var fotter_15=0;
                   $.each(data.row_data, function(i, item) {
                     var tot_ben_sc = isNaN(parseInt(item.tot_ben_sc)) ? 0 : parseInt(item.tot_ben_sc);
                     var total_verified_sc = isNaN(parseInt(item.total_verified_sc)) ? 0 : parseInt(item.total_verified_sc);
                     var total_yet_tobe_verified_sc = isNaN(parseInt(item.total_yet_tobe_verified_sc)) ? 0 : parseInt(item.total_yet_tobe_verified_sc);
                     var total_approved_sc = isNaN(parseInt(item.total_approved_sc)) ? 0 : parseInt(item.total_approved_sc);
                     var total_yet_tobe_approved_sc = isNaN(parseInt(item.total_yet_tobe_approved_sc)) ? 0 : parseInt(item.total_yet_tobe_approved_sc);
                     var total_rejected_sc = isNaN(parseInt(item.total_rejected_sc)) ? 0 : parseInt(item.total_rejected_sc);
                     var total_reverted_sc = isNaN(parseInt(item.total_reverted_sc)) ? 0 : parseInt(item.total_reverted_sc);
                     var total_verified1_sc=total_approved_sc+total_yet_tobe_approved_sc;

                     var tot_ben_st = isNaN(parseInt(item.tot_ben_st)) ? 0 : parseInt(item.tot_ben_st);
                     var total_verified_st = isNaN(parseInt(item.total_verified_st)) ? 0 : parseInt(item.total_verified_st);
                     var total_yet_tobe_verified_st = isNaN(parseInt(item.total_yet_tobe_verified_st)) ? 0 : parseInt(item.total_yet_tobe_verified_st);
                     var total_approved_st = isNaN(parseInt(item.total_approved_st)) ? 0 : parseInt(item.total_approved_st);
                     var total_yet_tobe_approved_st = isNaN(parseInt(item.total_yet_tobe_approved_st)) ? 0 : parseInt(item.total_yet_tobe_approved_st);
                     var total_rejected_st = isNaN(parseInt(item.total_rejected_st)) ? 0 : parseInt(item.total_rejected_st);
                     var total_reverted_st = isNaN(parseInt(item.total_reverted_st)) ? 0 : parseInt(item.total_reverted_st);
                     ...
                     table.append("<tr><td>"+(i+1)+"</td><td>"+item.location_name+"</td><td>"+tot_ben_sc+"</td><td>"+total_verified1_sc+"</td><td>"+total_yet_tobe_verified_sc+"</td><td>"+total_approved_sc+"</td><td>"+total_yet_tobe_approved_sc+"</td><td>"+total_rejected_sc+"</td><td>"+total_reverted_sc+"</td><td>"+tot_ben_st+"</td><td>"+total_verified1_st+"</td><td>"+total_yet_tobe_verified_st+"</td><td>"+total_approved_st+"</td><td>"+total_yet_tobe_approved_st+"</td><td>"+total_rejected_st+"</td><td>"+total_reverted_st+"</td></tr>");
                      //slno++;

                  });
                  
                  $("#example_new > tfoot #fotter_id").html("<th></th><th>Total</th><th>"+fotter_1+"</th><th>"+fotter_2+"</th><th>"+fotter_3+"</th><th>"+fotter_4+"</th><th>"+fotter_5+"</th><th>"+fotter_6+"</th><th>"+fotter_8+"</th><th>"+fotter_9+"</th><th>"+fotter_10+"</th><th>"+fotter_11+"</th><th>"+fotter_12+"</th><th>"+fotter_13+"</th><th>"+fotter_14+"</th><th>"+fotter_15+"</th>");
                  //$('#example tbody').empty();
                  $("#example").hide();
                   $("#example_new").show();
                   $("#exportToExcel").hide();
